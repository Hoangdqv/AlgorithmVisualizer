...
def selection_sort(arr, tracer):
    ...
    for i in range(n - 1):
        ...
        tracer.add_state(arr.copy(), selected=[i, min_index], variables={'i': i, 'min_index': min_index})
        # Swap the minimum element with the first element of unsorted portion
        if min_index != i:
            swap(arr, i, min_index)
            tracer.add_state(arr.copy(), swapped=[i, min_index], selected=[i, min_index], variables={'i': i, 'min_index': min_index})
